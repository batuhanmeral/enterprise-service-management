...
def _set_ticket_created(ticket, ts):
    ...
    SLA hedef tarihini (sla_due_at) de yeni created_at'e göre çalışma saatleri bazlı
    yeniden hesaplar — aksi halde demo biletlerin SLA rozetleri tutarsız olurdu."""
    hours = SLA_HOURS.get(ticket.priority, SLA_HOURS[Priority.NORMAL])
    Ticket.objects.filter(pk=ticket.pk).update(
        created_at=ts, updated_at=ts, sla_due_at=add_business_hours(ts, hours),
    )
