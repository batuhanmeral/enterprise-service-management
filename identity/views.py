...
# Audit log action prefix sabitleri — `log_ticket_action` çağrılarıyla senkron olmalı
TICKET_CLOSE_ACTION_PREFIX = 'Bilet kapatıldı'
TICKET_REOPEN_ACTION_PREFIX = 'Bilet yeniden açıldı'


def _build_user_resolution_stats(profile_user):
    """Bir personelin çözdüğü biletleri ve kategori başarı oranlarını hesaplar.

    Başarı kuralı: Kullanıcının bir bileti kapattığı son zaman damgasından sonra
    bilet yeniden açılmamışsa "başarılı"; açılmışsa "başarısız".
    """
    # Kullanıcının her bilete yaptığı son kapatma zamanı
    close_events = (
        TicketHistory.objects
        .filter(actor=profile_user, action__startswith=TICKET_CLOSE_ACTION_PREFIX)
        .values('ticket_id')
    ...
    reopen_events = (
        TicketHistory.objects
        .filter(ticket_id__in=ticket_ids, action__startswith=TICKET_REOPEN_ACTION_PREFIX)
...
class RegisterView(FormView):
    ...
    def form_valid(self, form):
        ...
        audit_log(self.request, AuditCategory.USER,
                  f'Yeni kullanıcı kaydı (onay bekliyor): {new_user.username}',
...
class UserListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Sıralama (?sort=username|-username|date_joined|-date_joined|role|department)
